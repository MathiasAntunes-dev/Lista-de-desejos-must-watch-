...
class Tarefa:
    ...
    def id(cls, id: int) -> Self:
        with Database() as db:
            query: str = 'SELECT titulo_tarefa, data_conclusao, tipo, concluida, data_finalizacao, indicado FROM tarefas WHERE id = ?;'
            params: tuple = (id,)
        ...
        return cls(id_tarefa=id, titulo_tarefa=titulo, data_conclusao=data, tipo=tipo, concluida=concluida, data_finalizacao=data_finalizacao, indicado=indicado)

    def salvar_tarefa(self: Self) -> None:
        with Database() as db:
            query: str = "INSERT INTO tarefas (titulo_tarefa, data_conclusao, tipo, indicado) VALUES (?, ?, ?, ?);"
            params: tuple = (self.titulo_tarefa, self.data_conclusao, self.tipo, self.indicado)
            db.executar(query, params)

    @classmethod
    def obter_tarefas(cls) -> list[Self]:
        with Database() as db:
            query: str = 'SELECT titulo_tarefa, data_conclusao, tipo, id, concluida, data_finalizacao, indicado FROM tarefas;'
            resultados: list[Any] = db.buscar_tudo(query)
            tarefas: list[Self] = [cls(titulo, data, tipo, id, concluida, data_finalizacao, indicado) for titulo, data, tipo, id, concluida, data_finalizacao, indicado in resultados]
            return tarefas
